...
export
function modeIconPath
(name) {
  return iconPath(modeIcon(name)?.name)
}
...
(buf, css, name, run) {
  ...
  (ele) {
    if (ele) {
      ...
      if (icon) {
        icon.firstElementChild.src = iconPath(name)
        if (run)
